...
fprintf('---------------------------------------------------------\n');
fprintf('      WOA: Whale Optimization Algorithm\n')
fprintf('---------------------------------------------------------\n');
...
% ----Modifiables----
bus_system = 33;            % [5, 33]
numDGs = 1;                 % [1, 2, 3] Recommended Maximum is 3             
...
baseFilename = sprintf('WOA_Results_%dBus_Hour%d_1DG.mat', bus_system, numHours);
resultVarName = sprintf('WOA_Result_%dBus_Hour%d', bus_system, numHours);
    ...
    if bus_system == 33
        switch numHours
            case {1, 3, 24, 96}
                ...
                resultVarName = sprintf('WOA_Result_%dBus_Hour%d', bus_system, numHours);
                optimalValueVarName = sprintf('WOA_Optimal_value_%dBus_Hour%d', bus_system, numHours);
                optimalPositionVarName = sprintf('WOA_Optimal_value_position_%dBus_Hour%d', bus_system, numHours);
                loadFlowResultsVarName = sprintf('WOA_Load_Flow_Results_%dBus_Hour%d', bus_system, numHours);
                columnIndexVarName = sprintf('WOA_column_index_%dBus_Hour%d', bus_system, numHours);
...
for j = (completedRuns + 1): run
    ...
    Runtime_Start = tic;
    Results = WOA(Population,MaxIt,numVars,numDGs,numHours,maxDGSize,lowerbound,upperbound,BDactive,BDreactive);
...
% Calculate and save the optimal value, position, etc.
...
if completedRuns == 100
    [Optimal_value_WOA, column_index_WOA] = min((loadedData.(resultVarName).Fitness));
    Optimal_value_position_WOA = loadedData.(resultVarName).Position(column_index_WOA,:);
    Load_Flow_Results_WOA = Load_Flow_Result(Optimal_value_position_WOA,numDGs,numHours,BDactive,BDreactive,linedata);

    loadedData.(optimalValueVarName) = Optimal_value_WOA;
    loadedData.(optimalPositionVarName) = Optimal_value_position_WOA;
    loadedData.(loadFlowResultsVarName) = Load_Flow_Results_WOA;
    loadedData.(columnIndexVarName) = column_index_WOA;
    save(baseFilename, '-struct', 'loadedData');
end
...
function displayWOAResults(WOA_Result, run, Optimal_value_position_WOA, numDGs, numHours)

    fprintf('---------------------------------------------------------\n');
    fprintf('The results are summarized as follows: \n');
    fprintf('    Number of runs = %d\n', run);
    fprintf('    Average = %s\n', three_decimal_format(mean(WOA_Result.Fitness)));
    fprintf('    Min (Optimal Solution Value) = %s kW\n', three_decimal_format(min(WOA_Result.Fitness)));
    fprintf('---------------------------------\n');

    locations = Optimal_value_position_WOA(1:numDGs);
    dispatches = reshape(Optimal_value_position_WOA(numDGs+1:end), numDGs, numHours);
